...
\newpage 
Les autres variables de la base de données sont des variables qualitatives dichotomiques ou polytomiques. Une analyse de fréquences de celles-ci permet d'avoir un meilleur ressenti quant à leurs interactions avec la variable exogène \textsc{Loss}:\\
<<echo = FALSE>>=
