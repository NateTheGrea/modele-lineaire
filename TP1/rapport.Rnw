...
\label{annex:transfo}
Dans la section \ref{sec:analyse}, il a été discuté qu'une transformation des variables continues serait à trouver. Pour ce faire, la méthode box-cox a d'abord été effectuer 
%<<echo = FALSE>>=
...
<<echo = FALSE>>=
par(mfrow=c(2,2))
p1 <- ggplot(data, aes(x=LOSS)) + ylab("Fréquence") + geom_histogram()
p2 <- ggplot(data, aes(x=log(LOSS))) + ylab("Fréquence")+ xlab("") + geom_histogram()
p2
grid.arrange(p6,p7,p8,p9,p10,p11, nrow = 3)
hist(data$LOSS)
hist(log(data$LOSS))
