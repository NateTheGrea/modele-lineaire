...
(Commentaires du beau Guillaume Michel ici; on aurait peut-être aussi pu rajouter le graphique avec la transformation, soit log(Loss) en fonction de CLMAGE)
...
De plus, voici le $R^2_a$ : $0.2754$. Ainsi, une grande variabilité de la variable endogène n'est pas expliquée par le modèle. Ceci est en parti dû au grand nombre de variables qualitatives dans le modèle qui n'ont pas assez de valeurs possibles afin de réfléter l'étendue des valeurs possible de la perte économique. (structure de la phrase à revoir) \\
